...
const CourseInfo = {
  ...
  let uniqueId = [...new Set(LearnerSubmissions.map(item=>item.learner_id))]
  console.log(uniqueId)

  let assignmentScore = [...new Set(LearnerSubmissions.map(item=>item.submission.score))]
  ...
  function getLearnerData(course, ag, submissions) {
    ...
    // ];
  let results = []
  for ( let i = 0; i <= uniqueId.length-1; i++){
    results.push ({learner_id:uniqueId[i]})
  }
  for (let i of LearnerSubmissions) {
    if (LearnerSubmissions.learner_id == results.learner_id){
        results.push(LearnerSubmissions[i].submissions.score)
    }
  }
